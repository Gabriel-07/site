...
class mh_affiliate_widget extends WP_Widget {
    function mh_affiliate_widget() {
        $widget_ops = array('classname' => 'mh_affiliate', 'description' => __('MH Affiliate Widget to earn money by promoting WordPress themes by MH Themes. For more information, please login to your account at mhthemes.com', 'mh'));
        $this->WP_Widget('mh_affiliate', __('MH Affiliate Widget', 'mh'), $widget_ops);
    }
    function widget($args, $instance) {
        extract($args);
        $title = apply_filters('widget_title', empty($instance['title']) ? '' : $instance['title'], $instance, $this->id_base);
        $mh_username = empty($instance['mh_username']) ? '' : $instance['mh_username'];
        $mh_ads = isset($instance['mh_ads']) ? $instance['mh_ads'] : '300x250';
                      
        echo $before_widget; 
        
        if (!empty($title)) { echo $before_title . $title . $after_title; } ?>
       	<a href="http://www.mhthemes.com/members/aff/go/<?php echo esc_attr($mh_username); ?>" target="_blank" title="Premium Magazine WordPress Themes"><img src="<?php echo get_template_directory_uri() . '/images/ads/mh_magazine_' . $mh_ads . '.png' ?>" alt="MH Magazine WordPress Theme" /></a> <?php
    ...
    function form($instance) {
        ...
        <p>
	    	<label for="<?php echo $this->get_field_id('mh_username'); ?>">MH Themes Username:</label>
