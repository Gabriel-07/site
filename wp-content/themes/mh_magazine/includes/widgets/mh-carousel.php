...
class mh_carousel_hp_widget extends WP_Widget {
    function mh_carousel_hp_widget() {
        $widget_ops = array('classname' => 'mh_carousel_hp', 'description' => __('Carousel widget for use on homepage templates', 'mh'));
        $this->WP_Widget('mh_carousel_hp', __('MH Carousel Widget (Homepage)', 'mh'), $widget_ops);
    }
    function widget($args, $instance) {
        extract($args);
        $title = apply_filters('widget_title', empty($instance['title']) ? __('Latest Posts', 'mh') : $instance['title'], $instance, $this->id_base);
