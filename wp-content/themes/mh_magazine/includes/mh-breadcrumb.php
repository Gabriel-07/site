	...
	function mh_breadcrumb() {
		if (!is_home() && !is_front_page()) {
			...
			if (isset($options['breadcrumbs']) && $options['breadcrumbs']) {	
				...
				$close_span = '</span>';
				echo '<div class="breadcrumb">' . $before_link . '<a href="' . esc_url(home_url()) . '" itemprop="url">' . $before_title . __('Home', 'mh') . $close_span . '</a>' . $close_span . $delimiter;	
				if (is_single() && get_post_type() == 'post' && !is_attachment()) {
					...
					echo __('Authors', 'mh') . $delimiter . esc_attr($user_info->display_name) . '</div>' . "\n";							
				} elseif (is_404()) {
					echo __('Page not found (404)', 'mh') . '</div>' . "\n";
				} elseif (is_search()) {
					echo __('Search', 'mh') . $delimiter . get_search_query() . '</div>' . "\n";
				} elseif (is_date()) {
					...
					if (is_day()) {
						...
						echo $before_link . '<a href="' . $url_year . '" title="' . __('Yearly Archives', 'mh') . '" itemprop="url">' . $before_title . $arc_year . $close_span . '</a>' . $close_span . $delimiter . $arc_month . '</div>' . "\n";
					} elseif (is_year()) {
						echo $arc_year . '</div>' . "\n";
					}
